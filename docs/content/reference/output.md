...
describes what users get. Pick a format with `-o`, or let imo-shortlist choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
imo-shortlist <command> -o table   # aligned columns for reading
imo-shortlist <command> -o jsonl   # one JSON object per line, for piping
imo-shortlist <command> -o json    # a single JSON array
imo-shortlist <command> -o csv     # spreadsheet friendly
imo-shortlist <command> -o tsv     # tab-separated
imo-shortlist <command> -o url     # just the URL column
imo-shortlist <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
imo-shortlist <command> --fields id,title,url
```
...
```bash
imo-shortlist <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
imo-shortlist <command>            # a table, because this is a terminal
imo-shortlist <command> | wc -l    # JSONL, because this is a pipe
```
